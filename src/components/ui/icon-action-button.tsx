...
/** 顯示在按鈕上方，避免在 overflow 容器（例如橫向捲動表格）內被底部裁切 */
const TOOLTIP_CLASS =
    "pointer-events-none absolute left-1/2 bottom-full z-[100] mb-1.5 -translate-x-1/2 whitespace-nowrap rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--panel))] px-2 py-1 text-xs text-[rgb(var(--text))] opacity-0 shadow-sm transition group-hover:opacity-100 group-focus-within:opacity-100";
        ...
        return (
            <span className="group relative z-10 inline-flex">
                <Link
                    ...
                    title={tooltipText}
                    className={cn(BASE_BUTTON_CLASS, className)}
                    {...linkProps}
                >
