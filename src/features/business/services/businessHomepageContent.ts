...
export const DEFAULT_BUSINESS_HOMEPAGE_CONTENT_STATE: BusinessHomepageContentState = {
    ...
    locale: {
        ...
        en: {
            ...
            ctaSecondary: "Get test credentials",
...
function normalizeSecondaryCta(value: unknown, fallback: string): string {
    const next = sanitizeText(value, fallback);
    if (next === "認識我的背景") return "取得測試帳密";
    if (next === "Read my background") return "Get test credentials";
    return next;
}
...
function normalizeLocaleContent(candidate: unknown, fallback: BusinessHomepageEditableContent): BusinessHomepageEditableContent {
    ...
    return {
        ...
        ctaSecondary: normalizeSecondaryCta(source.ctaSecondary, fallback.ctaSecondary),
